...
export function NewConversationDialog({ open, mode, bots, onClose, onCreated }: Props) {
  ...
  return (
    <div
      className="fixed inset-0 bg-[rgba(15,23,42,0.25)] z-40 flex items-center justify-center"
      ...
      <div className="bg-white rounded-[14px] w-[480px] max-h-[80vh] overflow-y-auto shadow-[0_20px_60px_rgba(0,0,0,0.15)] p-6">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-[17px] font-semibold">
              {mode === "single" ? "新建私聊" : "新建群聊"}
            </h2>
            <p className="text-[13px] text-[#64748B] mt-1">
              {mode === "single" ? "选择一个 Bot 开始对话" : "选择多个 Bot 开始协作"}
            </p>
          </div>
          <button
            onClick={onClose}
            className="w-[30px] h-[30px] rounded-[7px] flex items-center justify-center text-[#94A3B8] hover:bg-[#F1F5F9] hover:text-[#475569] transition-colors"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <label className="text-[13px] font-medium block mb-2">
          ...
          <span className="text-[#94A3B8] font-normal">
            {mode === "group" && "（可多选，至少 2 个）"}
          </span>
        </label>
        <div className="space-y-2 mb-5">
          {bots.map((bot) => {
            const checked =
              mode === "single" ? selectedBotId === bot.id : selectedBotIds.includes(bot.id);
            return (
              <div
                key={bot.id}
                onClick={() => {
                  if (mode === "single") {
                    ...
                    setSelectedBotIds((ids) =>
                      ids.includes(bot.id) ? ids.filter((i) => i !== bot.id) : [...ids, bot.id],
                    );
        ...
        {mode === "group" && selectedBotIds.length >= 2 && (
          <div className="mb-5">
            <label className="text-[13px] font-medium block mb-2">
              设置主 Bot{" "}
              <span className="text-[#94A3B8] font-normal">（接收用户消息，统筹协作）</span>
            </label>
            <select
              value={primaryBotId}
              onChange={(e) => setPrimaryBotId(e.target.value)}
        ...
        <input
          ...
          placeholder="留空自动生成"
          className="w-full px-3 py-2 text-[14px] border border-[#E5E7EB] rounded-lg bg-white outline-none focus:border-[#93C5FD] transition-colors mb-5"
        />
        ...
        <div className="flex justify-end gap-2">
          ...
          <button
            ...
            disabled={
              createMut.isPending ||
              (mode === "single" ? !selectedBotId : selectedBotIds.length < 2)
            }
