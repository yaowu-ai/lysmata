        ...
        <div className="flex flex-1 overflow-hidden min-w-0">
            ...
            <div className="bg-white border-b border-[#E5E7EB] px-5 py-3 flex items-center gap-3 flex-shrink-0">
              ...
              <div className="flex-1 min-w-0 ml-1">
                ...
                <div className="text-[12px] text-[#64748B]">
                  输入 <code className="font-mono bg-[#F1F5F9] px-1 rounded text-[11px]">@</code> 可指定 Bot 响应
                </div>
              </div>
              <a href="/bots" className="w-8 h-8 rounded-[7px] flex items-center justify-center text-[#94A3B8] hover:bg-[#F1F5F9] hover:text-[#475569] transition-colors" title="Bot 管理">
                <Settings size={15} />
              </a>
            </div>
          ...
          <aside className="w-[280px] bg-white border-l border-[#E5E7EB] flex flex-col flex-shrink-0 overflow-y-auto">
            ...
            <div className="px-2 py-2">
              <div className="text-[11px] font-semibold text-[#94A3B8] uppercase tracking-wider px-2 py-1.5">参与 Bot</div>
              {convBots.map((b) => {
                const isPrimary = b.id === primaryBotId;
                return (
                  <div key={b.id} className="flex items-center gap-2.5 px-3 py-2 rounded-lg hover:bg-[#F8FAFC]">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className={`text-[10px] px-1.5 py-0.5 rounded-[20px] font-semibold border ${isPrimary ? 'bg-[#FEF3C7] text-[#92400E] border-[#FDE68A]' : 'bg-[#F1F5F9] text-[#64748B] border-[#E5E7EB]'}`}>
                          {isPrimary ? '👑 主Bot' : '辅助'}
                        </span>
                      </div>
            ...
            <div className="px-4 py-3 border-t border-[#F1F5F9]">
              <div className="text-[11px] font-semibold text-[#94A3B8] uppercase tracking-wider mb-2">感知注入</div>
              <div className="bg-[#FFFBEB] border border-[#FDE68A] rounded-lg px-3 py-2.5 text-[12px] leading-[1.6] text-[#78350F]">
                <strong>[群聊上下文]</strong> 当前群聊中还有以下 Bot 可以协作：<br />
                {convBots.filter((b) => b.id !== primaryBotId).map((b) => (
                  <span key={b.id} className="block">• <strong>@{b.name}</strong>：{b.description || b.name}</span>
                ))}
              </div>
            </div>
          </aside>
