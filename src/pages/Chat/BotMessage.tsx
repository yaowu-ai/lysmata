...
export function BotMessage({ message, isPrimary }: Props) {
  ...
  const [resolvedState, setResolvedState] = useState<"pending" | "approved" | "rejected">(
    "pending",
  );
            ...
            <div className="p-3">
              <div className="mb-2">
                ...
                <code className="bg-[#F1F5F9] px-1.5 py-0.5 rounded text-[#0F172A]">
                  {((metadata.call as Record<string, unknown>)?.command as string) ||
              ...
              <div className="mb-3">
                ...
                <pre className="bg-[#1E293B] text-[#E2E8F0] p-2 rounded-md overflow-x-auto text-[12px]">
                  {JSON.stringify(
                    (metadata.call as Record<string, unknown>)?.args || metadata.args || {},
                    null,
                    2,
                  )}
                </pre>
