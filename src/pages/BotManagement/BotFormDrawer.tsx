...
import { X, Plus, Trash2, Wifi, CheckCircle, XCircle } from 'lucide-react';
...
export function BotFormDrawer({ open, bot, onClose }: Props) {
  ...
  useEffect(() => {
    if (bot) {
      ...
      const s = Array.isArray(bot.skills_config) ? bot.skills_config : JSON.parse(bot.skills_config as unknown as string);
      setSkills(s);
      setMcpJson(typeof bot.mcp_config === 'string' ? bot.mcp_config : JSON.stringify(bot.mcp_config, null, 2));
      setWsUrl(bot.openclaw_ws_url);
      setWsToken(bot.openclaw_ws_token ?? '');
      setIsActive(!!bot.is_active);
    } else {
      setName(''); setEmoji('🤖'); setDescription(''); setSkills([]);
      setMcpJson('{}'); setWsUrl('ws://localhost:18789/ws'); setWsToken(''); setIsActive(true);
    }
    setTab('基础');
    setTestResult(null);
  }, [bot, open]);
  ...
  async function handleSave() {
    const payload = {
      name, avatar_emoji: emoji, description,
      skills_config: skills, mcp_config: (() => { try { return JSON.parse(mcpJson); } catch { return {}; } })(),
      openclaw_ws_url: wsUrl, openclaw_ws_token: wsToken || undefined, is_active: isActive,
    };
  ...
  return (
    <aside
      className={cn(
        'fixed top-0 right-0 bottom-0 w-[480px] bg-white z-30 flex flex-col shadow-[-8px_0_32px_rgba(0,0,0,0.08)] transition-transform duration-250 ease-in-out',
    ...
    >
      ...
      <div className="px-6 py-4 border-b border-[#E5E7EB] flex items-center justify-between flex-shrink-0">
        <div>
          ...
          <p className="text-[12px] text-[#64748B] mt-0.5">配置 OpenClaw Agent 连接信息</p>
        </div>
        <button onClick={onClose} className="w-8 h-8 rounded-[7px] flex items-center justify-center text-[#94A3B8] hover:bg-[#F1F5F9] hover:text-[#475569] transition-colors">
          <X size={16} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <>
            <Field label="Bot 名称" required>
              <input value={name} onChange={(e) => setName(e.target.value)}
                placeholder="例如：CodeMaster" className={inputCls} />
            </Field>
            <Field label="头像 Emoji">
              <input value={emoji} onChange={(e) => setEmoji(e.target.value)}
                placeholder="🤖" className={cn(inputCls, 'text-xl')} />
            </Field>
            <Field label="能力描述" hint="将在群聊感知注入中展示给其他 Bot">
              <textarea value={description} onChange={(e) => setDescription(e.target.value)}
                rows={3} placeholder="擅长代码审查与 TypeScript 最佳实践..."
                className={cn(inputCls, 'resize-none')} />
            </Field>
            <div className="flex items-center gap-2">
              <input type="checkbox" id="is_active" checked={isActive} onChange={(e) => setIsActive(e.target.checked)} className="w-4 h-4 accent-blue-600" />
              <label htmlFor="is_active" className="text-[14px] text-[#0F172A]">启动时自动建立 WS 连接</label>
            </div>
          </>
        )}
        ...
        {tab === 'Skills' && (
          <>
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setSkills([...skills, { name: '', description: '' }])}
            ...
            {skills.map((s, i) => (
              <div key={i} className="flex gap-2 items-start">
                <div className="flex-1 space-y-2">
                  <input value={s.name} onChange={(e) => { const n = [...skills]; n[i] = { ...n[i], name: e.target.value }; setSkills(n); }}
                    placeholder="技能名称" className={inputCls} />
                  <input value={s.description} onChange={(e) => { const n = [...skills]; n[i] = { ...n[i], description: e.target.value }; setSkills(n); }}
                    placeholder="一行描述" className={inputCls} />
                </div>
        ...
        {tab === 'MCP' && (
          <Field label="MCP 配置 JSON" hint="填写 MCP 服务器配置">
            <textarea value={mcpJson} onChange={(e) => setMcpJson(e.target.value)}
        ...
        {tab === '连接' && (
          <>
            <Field label="OpenClaw WS URL" required hint="格式：ws://host:port/ws">
              <input value={wsUrl} onChange={(e) => setWsUrl(e.target.value)}
                placeholder="ws://localhost:18789/ws" className={cn(inputCls, 'font-mono text-[13px]')} />
            </Field>
            <Field label="鉴权 Token" hint="可选，填写后以 ?token=xxx 追加到 WS URL">
              <input value={wsToken} onChange={(e) => setWsToken(e.target.value)}
                type="password" placeholder="留空则不鉴权" className={inputCls} />
            </Field>
            {isEdit && (
              <div className="space-y-2">
                ...
                {testResult && (
                  <div className={cn('flex items-center gap-2 text-[13px] px-3 py-2 rounded-lg border',
                    testResult.success ? 'bg-[#F0FDF4] border-[#BBF7D0] text-[#15803D]' : 'bg-[#FEF2F2] border-[#FECACA] text-[#B91C1C]')}>
      ...
      <div className="px-6 py-4 border-t border-[#E5E7EB] flex justify-end gap-2 flex-shrink-0">
        ...
        <button onClick={handleSave} disabled={isPending || !name || !wsUrl}
...
const inputCls = 'w-full px-3 py-2 text-[14px] border border-[#E5E7EB] rounded-lg bg-white outline-none focus:border-[#93C5FD] focus:shadow-[0_0_0_3px_rgba(147,197,253,0.25)] transition-all';
