...
border-radius: 20px;
font-size: 11px ~ 12px;  font-weight: 500 ~ 600;
```
...
```css
.bubble-primary {
  background: #F0F7FF;  color: #0F172A;
...
.toast-error   { background: #fff;  color: #B91C1C;  border-left: 4px solid #DC2626; }
```
...
.mrow { animation: mfade 0.2s ease; }
...
/* Nav */
#left-nav { width: 64px; flex-shrink: 0; }
#left-nav.expanded { width: 220px; }

/* 会话侧边栏 */
aside.conv-sidebar { width: 260px; flex-shrink: 0; }

/* 聊天主区域 */
main { flex: 1; display: flex; flex-direction: column; overflow: hidden; min-width: 0; }

/* 右侧信息面板（常驻型）*/
aside.info-panel { width: 280px; flex-shrink: 0; }

/* 右侧 Artifact 面板（常驻型，更宽）*/
aside.artifact-panel { width: 480px; flex-shrink: 0; }

/* 右侧抽屉（Bot 管理，fixed 覆盖型）*/
.drawer { position: fixed; right: 0; top: 0; bottom: 0; width: 480px; z-index: 30; }
```
